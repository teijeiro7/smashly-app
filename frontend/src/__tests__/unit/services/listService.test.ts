...
const mockDbLists = [
  { id: 'list-1', user_id: 'user-1', name: 'Mis Favoritas', description: 'Favs description', created_at: '2025-01-01T00:00:00Z', updated_at: '2025-01-01T00:00:00Z', is_public: false, racket_count: [{ count: 5 }] },
  { id: 'list-2', user_id: 'user-1', name: 'Para Comprar', description: null, created_at: '2025-01-02T00:00:00Z', updated_at: '2025-01-02T00:00:00Z', is_public: true, racket_count: [{ count: 3 }] },
]

const mockDbListWithRackets = [
  {
    id: 'list-1', user_id: 'user-1', name: 'Mis Favoritas', description: 'Favs description', created_at: '2025-01-01T00:00:00Z', updated_at: '2025-01-01T00:00:00Z', is_public: false,
    rackets: [
...
function qb(d: any, e: any = null) {
  const c: any = new Proxy({ _d: d, _e: e }, {
    get(t, p) {
...
describe('ListService', () => {
  ...
  describe('createList', () => {
    it('should throw if no session', async () => {
      ...
      await expect(ListService.createList({ name: 'Test' })).rejects.toThrow('No hay sesión activa')
    })

    it('should create and return list', async () => {
      const newListData = { id: 'new-1', user_id: 'u1', name: 'Nueva Lista', description: null, created_at: '2025-03-01T00:00:00Z', updated_at: '2025-03-01T00:00:00Z', is_public: false }
      seed([newListData])
  ...
  describe('updateList', () => {
    ...
    it('should throw on error', async () => {
      ...
      await expect(ListService.updateList('bad-id', { name: 'Test' })).rejects.toThrow('Update failed')
    })
  })
  ...
  describe('addRacketToList', () => {
    ...
    it('should ignore duplicate error', async () => {
      mockFrom.mockImplementationOnce(() => ({
        insert: vi.fn(() => ({
          then: (resolve: any) => resolve({ data: null, error: { code: '23505', message: 'duplicate' } }),
        })),
      }))
      await expect(ListService.addRacketToList('list-1', 1)).resolves.not.toThrow()
    })
    ...
    it('should throw on other error', async () => {
      mockFrom.mockImplementationOnce(() => ({
        insert: vi.fn(() => ({
          then: (resolve: any) => resolve({ data: null, error: { code: '23506', message: 'FK violation' } }),
        })),
      }))
